        ...
        <div class="flex">
            ...
            <div class="w-3/4">
                <div class="p-4 flex justify-between items-center">
                    <h1 class="text-lg">Добавить промокод</h1>
                    <Link :href="route('promocodes.index')" class="inline-block text-xs px-3 py-2 bg-red-500 text-white">Назад</Link>
                </div>
                <div v-if="storeSuccess && Object.keys(promocode).length === 0" class="bg-green-600 text-white p-4">
...
<!--                    </div>-->
                    <div class="mb-4">
                        <a @click.prevent="storePromocode" href="#" class="inline-block text-xs px-3 py-2 bg-green-600 text-white">Добавить</a>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        storePromocode() {
            axios.post('/admin/promocodes', this.promocode)
                .then(res => {
                    this.promocode = {}
